...
import cn.edu.bupt.sdmda.sort.SortAlgorithm;
...
public class DSMain {
	public static void main(String[] args) {
		...
		if (args.length >= 2) {
			...
			case "calculator":
				...
				break;
			case "convert":
				convert(args[1], Integer.parseInt(args[2]), Integer.parseInt(args[3]));
				break;
			case "bracketsmatch":
				boolean res = false;
				if (args.length == 2) {
	...
	public static void testSort(String[] args) {
		...
		for (int i = 0; i < data; i++) {
			list.add(rd.nextDouble() * data);
		}
		testInsert(data, frequency, list);
		testSelect(data, frequency, list);
		testBubble(data, frequency, list);
		testQsort(data, frequency, list);
		testMerge(data, frequency, list);
	}

	public static void testMerge(int data, int frequency, ArrayList<Double> list) {
		long startTime = System.currentTimeMillis();
		for (int i = 0; i < frequency; i++) {
			ArrayList<Double> tmp = new ArrayList<Double>(list);
			SortAlgorithm.mergeSort(tmp, 0, tmp.size() - 1);
		}
		long endTime = System.currentTimeMillis();
		long usedTime = (endTime - startTime) / 100;
		System.out.println("Merge:" + usedTime);
	}

	public static void testQsort(int data, int frequency, ArrayList<Double> list) {
		long startTime = System.currentTimeMillis();
		for (int i = 0; i < frequency; i++) {
			ArrayList<Double> tmp = new ArrayList<Double>(list);
			SortAlgorithm.QSort(tmp, 0, tmp.size() - 1);
		}
		long endTime = System.currentTimeMillis();
		long usedTime = (endTime - startTime) / 100;
		System.out.println("Qsort:" + usedTime);
	}

	public static void testSelect(int data, int frequency, ArrayList<Double> list) {
		long startTime = System.currentTimeMillis();
		for (int i = 0; i < frequency; i++) {
			ArrayList<Double> tmp = new ArrayList<Double>(list);
			SortAlgorithm.selectionSort(tmp);
		}
		long endTime = System.currentTimeMillis();
		long usedTime = (endTime - startTime) / 100;
		System.out.println("Selection:" + usedTime);
	}

	public static void testBubble(int data, int frequency, ArrayList<Double> list) {
		long startTime = System.currentTimeMillis();
		for (int i = 0; i < frequency; i++) {
			ArrayList<Double> tmp = new ArrayList<Double>(list);
			SortAlgorithm.bubbleSort(tmp);
		}
		long endTime = System.currentTimeMillis();
		long usedTime = (endTime - startTime) / 100;
		System.out.println("Bubble:" + usedTime);
	}

	public static void testInsert(int data, int frequency, ArrayList<Double> list) {
		long startTime = System.currentTimeMillis();
		for (int i = 0; i < frequency; i++) {
			ArrayList<Double> tmp = new ArrayList<Double>(list);
			SortAlgorithm.insertSort(tmp);
		}
		long endTime = System.currentTimeMillis();
		long usedTime = (endTime - startTime) / 100;
		System.out.println("Insert:" + usedTime);
	}
